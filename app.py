...
def image_to_text(path):
    ...
    # Optional: Raise error if OCR failed
    if len(text.strip()) < 10:
        raise ValueError("OCR failed to extract meaningful text. Please try a clearer or printed image.")
...
def gramatical_accuracy(extracted_text):
    original = TextBlob(extracted_text)
    corrected = original.correct()
    ...
    return ((len(original_words) - differences) / len(original_words)) * 100
...
def percentage_of_corrections(extracted_text):
    # Simulate corrections locally using TextBlob's spelling correction
    ...
    # Count number of different words (i.e., corrected words)
    corrections = sum(1 for o, c in zip(original_words, corrected_words) if o != c)

    # Normalize by number of words
    return corrections / len(original_words) * 100
...
def score(input):
    spelling, grammar, correction, phonetic = input

    # Normalize and weight scores
    score = 0
    score += spelling * 0.3
    score += grammar * 0.2
    score += (100 - correction) * 0.2
    score += phonetic * 0.3

    # Max score: close to 100. Higher = better performance
    return [1.0, 0.0] if score >= 75 else [0.0, 1.0]
...
st.set_page_config(page_title="Fluento: AI Powered Early Detection of Dyslexia")
...
st.header("Fluento: AI Powered Early Detection of Dyslexia")
...
with tab2:
    ...
    with st.container():
        ...
        if st.button("Predict", help="Click after uploading the correct image"):
           try:
               st.info("Processing image and extracting features...")
               feature_array = get_feature_array("temp.jpg")
               st.write("Feature Scores:")
               st.write({
                   "Spelling Accuracy (%)": feature_array[0],
                   "Grammar Accuracy (%)": feature_array[1],
                   "Correction Load (%)": feature_array[2],
                   "Phonetic Accuracy (%)": feature_array[3]
               })
       
               result = score(feature_array)
               st.write("Prediction Score:", result)

               if result[0] == 1.0:
                   st.success("🟢 Likely NOT dyslexic. Writing features appear strong.")
               else:
                   st.warning("🔴 Likely DYSLEXIC. Errors suggest possible dysgraphia/dyslexia.")
    ...
    def listen_for(seconds: int):
      ...
      with sr.Microphone() as source:
          ...
          try:
              ...
              st.success("✅ Done listening, now recognizing speech...")
              ...
              st.error(f"❌ Could not request results; check your internet. {e}")
              return ""
          except Exception as e:
